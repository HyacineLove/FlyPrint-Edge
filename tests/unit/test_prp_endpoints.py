...
class PRPEndpointTests(unittest.TestCase):
    def setUp(self):
        ...
        self.portal.bind(self.session_id, {
            ...
            "display_name": "User", "claim_base_url": "https://portal.example.test",
    ...
    def test_list_requires_current_portal_session(self):
        with patch.object(main, "interactive_session_manager", self.interactive), \
            ...
            response = asyncio.run(main.list_prp_provider_files("prp-a", "wrong-session"))
    ...
    def test_list_timeout_returns_a_gateway_timeout_with_a_stable_error_code(self):
        with patch.object(main, "interactive_session_manager", self.interactive), \
            ...
            response = asyncio.run(main.list_prp_provider_files("prp-a", self.session_id, page=1, page_size=6))
    ...
    def test_select_downloads_and_binds_only_current_session(self):
        with patch.object(main, "interactive_session_manager", self.interactive), \
            ...
            response = asyncio.run(
                main.select_prp_provider_file("prp-a", "file-1", _Request({"session_id": self.session_id}))
            )
    ...
    def test_select_binds_downloaded_image_source_path(self):
        ...
        self.assertEqual(".png", self.selections.get_source(self.session_id, "file-1", "prp-a").suffix)
    ...
    def test_select_binds_downloaded_docx_source_path(self):
        ...
        self.assertEqual(".docx", self.selections.get_source(self.session_id, "file-1", "prp-a").suffix)
    ...
    def test_clear_selection_preserves_portal_session_and_keeps_session_caches(self):
        ...
        self.assertIsNotNone(self.selections.activate_cached(self.session_id, "file-1", "prp-a"))
