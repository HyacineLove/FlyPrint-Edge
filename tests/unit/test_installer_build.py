...
class InnoSetupScriptTests(unittest.TestCase):
    ...
    def test_uninstaller_stops_runtime_before_deleting_files(self):
        ...
        self.assertNotIn("CloseApplicationsFilter=", text)
        self.assertIn("function PrepareToInstall", text)
        self.assertIn("ewWaitUntilTerminated", text)
        self.assertIn("function WaitForLauncherExit", text)
        self.assertIn("CheckForMutexes('{#MyLauncherMutex}')", text)
        self.assertIn("if not WaitForLauncherExit then", text)
        wait_check = text.index("if not WaitForLauncherExit then")
        result_code_check = text.index("if ResultCode <> 0 then")
        self.assertLess(wait_check, result_code_check)
